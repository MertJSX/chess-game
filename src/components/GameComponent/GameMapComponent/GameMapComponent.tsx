...
import {
  colPlaceForCoordinate,
  rowPlaceForCoordinate,
} from "../utils/placeForCoordinate";
...
const GameMapComponent: React.FC<GameMapProps> = ({
  ...
  markedItems,
  setMarkedItems,
  markedPossibleCastles,
  setMarkedPossibleCastles,
  chessGame,
  setChessGame,
}) => {
  const [arrayFromGameMap, setArrayFromGameMap] = useState<Array<MapFrame>>(
    Array.from(gameMap.mapFrames.values())
  );
        ...
        {arrayFromGameMap.map((item) => (
          <article
            ...
            className={`frame relative flex justify-center items-center text-xl w-10 md:w-20 h-10 md:h-20 transition-all
                  ${
                    item.isSelected
                      ? "bg-sky-600 selected-frame"
                      : item.isThreatened
                      ? `bg-red-600 checked-frame`
                      : item.color === "black" &&
                        (item.isMarked || item.isMarkedForCastle)
                      ? `${
                          item.piece
                            ? "bg-chess-black-blue-2 hover:bg-sky-500"
                            : "hover:bg-chess-black-blue"
                        } bg-chess-black`
                      : item.color === "white" &&
                        (item.isMarked || item.isMarkedForCastle)
                      ? `${
                          item.piece
                            ? "bg-chess-white-blue-2 hover:bg-sky-500"
                            : "hover:bg-chess-white-blue"
                        } bg-chess-white`
                      : item.color === "black"
                      ? "bg-chess-black"
                      : "bg-chess-white"
          ...
          >
            {colPlaceForCoordinate(item, flipBoard) ? (
              <h1
                className={`absolute z-0 md:m-1 text-xs md:text-sm font-semibold select-none bottom-0 left-0 w-5 md:w-10 ${
                  item.color === "black"
                    ? "text-chess-white"
                    : "text-chess-black"
                }`}
            ...
            {rowPlaceForCoordinate(item, flipBoard) ? (
              <h1
                className={`absolute z-0 md:m-1 text-xs md:text-sm font-semibold select-none top-0 right-0 w-5 md:w-10 text-right ${
                  item.color === "black"
                    ? "text-chess-white"
                    : "text-chess-black"
                }`}
            ...
            {item.piece ? (
              <img
                className="select-none z-10 transition-all"
                src={item.piece.image}
                alt=""
                width={70}
              />
            ) : item.isMarked && hoveredItem !== item.positionName ? (
              <TbPointFilled size={55} className="text-cyan-500" />
            ) : null}
          </article>
        ))}
      </section>
      {chessGame.gameMode === "1v1" ? (
        <h1 className="text-center text-white text-2xl">
          {chessGame.turn === "black" ? (
            ...
            <span className="text-slate-100 font-bold">WHITE</span>
          )}{" "}
          turn
        </h1>
      ) : null}
